...
manifest_types='application/vnd.docker.distribution.manifest.v2+json, application/vnd.docker.distribution.manifest.list.v2+json, application/vnd.oci.image.manifest.v1+json, application/vnd.oci.image.index.v1+json'
...
manifests_request = 'https://{}/v2/{}/{}/manifests/{}'.format(registry_url, repository, img, tag)
if (mode == "debug"):
    print("manifests_request:")
    print(manifests_request)
    print("auth headers:")
    print(auth_head)

resp = requests.get(manifests_request, headers=auth_head, verify=False)
...
if (resp.status_code != 200):
    ...
    auth_head = get_auth_head(registry_endpoint,manifest_types)
    generated_req = 'https://{}/v2/{}/{}/manifests/{}'.format(registry_url, repository, img, tag)
    if (mode == "debug"):
        print("generated_req:")
        print(generated_req)
        print("auth headers:")
        print(auth_head)
    resp = requests.get(generated_req, headers=auth_head, verify=False)
...
for layer in layers:
    ...
    sys.stdout.flush()
    auth_head = get_auth_head(registry_endpoint,manifest_types) # refreshing token to avoid its expiration
